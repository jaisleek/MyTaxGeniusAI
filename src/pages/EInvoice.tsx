...
import { Receipt, Plus, Download, Send, CheckCircle, Shield, WifiOff, Save, Loader2 } from 'lucide-react';
...
export default function EInvoice() {
  ...
  useEffect(() => {
    const fetchDraft = async (userId: string) => {
      try {
        const docRef = doc(db, 'invoices', `${userId}_draft`);
        ...
        if (docSnap.exists()) {
          ...
          setSavedAt(new Date(data.updatedAt?.toMillis() || Date.now()));
        } else {
           loadLocalDraft();
        }
      } catch (e) {
        console.error("Failed to load invoice draft from cloud", e);
        loadLocalDraft();
      }
    };
    ...
    const loadLocalDraft = () => {
      ...
      if (saved) {
        try {
          ...
          console.error("Failed to load invoice draft locally", e);
        }
      }
    };
  ...
  // Save to database or local storage automatically
  useEffect(() => {
    const saveState = async () => {
       setIsSyncing(true);
       const state = { items, customerName, customerEmail, subtotal, vat, total, status: 'Draft' };
       localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
       
       const user = auth.currentUser;
       if (user && !isOffline) {
          try {
             await setDoc(doc(db, 'invoices', `${user.uid}_draft`), {
                ...state,
                userId: user.uid,
  ...
  }, [items, customerName, customerEmail, subtotal, vat, total, isOffline]);
          ...
          <div className="bg-white dark:bg-slate-100 p-8 rounded-3xl shadow-xl border border-gray-200 h-full flex flex-col relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-10 pt-4">
              ...
              <div className="text-right">
                ...
                <p className="text-gray-500 text-sm">Tax ID: 23145690-0001</p>
              </div>
            </div>
