...
export default function Dashboard() {
  ...
  const [filterCategory, setFilterCategory] = useState<'All' | 'SME' | 'Individual'>('All');
  ...
  const complianceScore = useMemo(() => {
    let score = 0;
    if (taxId) score += 35; 
    if (userName && userName !== 'Taxpayer') score += 15; 
    if (expenses.length > 0) score += 25; 
    if (securityEmail) score += 25; 
    return score;
  }, [taxId, userName, expenses, securityEmail]);
  ...
  const resetIdleTimer = useCallback(() => {
    if (showSessionWarning) return; // Don't reset if warning is already showing

    if (idleTimerRef.current) clearTimeout(idleTimerRef.current);
    if (countdownIntervalRef.current) clearInterval(countdownIntervalRef.current);
    
    // Set for 14 minutes (840,000 ms) before showing warning. 
  ...
  }, [showSessionWarning, handleLogout]);
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 z-40 shadow-sm relative">
          <div className="flex items-center text-slate-800 font-bold text-lg">
            ...
            {localStorage.getItem("isPro") === "true" && (
              <span className="ml-3 bg-emerald-100 text-emerald-700 text-xs font-black uppercase px-2.5 py-1 rounded-full border border-emerald-200">
                PRO
              </span>
            )}
          </div>
          ...
          <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div>
                ...
                <p className="text-slate-500 font-medium pb-2">Here's a summary of your current tax standing.</p>
                <div className="inline-flex bg-slate-100 p-1 rounded-xl items-center text-sm font-bold shadow-sm">
                  <button onClick={() => setFilterCategory('All')} className={`px-4 py-1.5 rounded-lg transition-colors ${filterCategory === 'All' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Overview</button>
                  <button onClick={() => setFilterCategory('Individual')} className={`px-4 py-1.5 rounded-lg transition-colors ${filterCategory === 'Individual' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>Individual / PIT</button>
                  <button onClick={() => setFilterCategory('SME')} className={`px-4 py-1.5 rounded-lg transition-colors ${filterCategory === 'SME' ? 'bg-white text-emerald-700 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>SME / Corporate</button>
                </div>
              </div>
              {taxId && (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <div 
                onClick={() => navigate('/chat')}
                className="cursor-pointer bg-gradient-to-r from-emerald-900 to-slate-900 rounded-3xl p-6 text-white relative overflow-hidden flex items-center shadow-lg hover:shadow-xl transition-all group lg:col-span-1"
              >
                ...
                <div className="relative z-10 flex items-center pr-10">
                  ...
                  <div>
                    <h3 className="text-lg font-bold mb-0.5">TaxBuddy Web</h3>
                    <p className="text-emerald-100/80 font-medium text-xs sm:text-sm">
                      Chat with TaxBuddy on the web to ask questions.
                    </p>
                  </div>
                </div>
              ...
              {/* WhatsApp Bot Banner */}
              <div 
                onClick={() => window.open('https://wa.me/15556547445?text=Hi%20MyTaxGenius%20AI', '_blank')}
                className="cursor-pointer bg-[#25D366] hover:bg-[#20b858] rounded-3xl p-6 text-white relative overflow-hidden flex items-center shadow-lg hover:shadow-xl transition-all group lg:col-span-1"
              >
                <div className="absolute right-0 bottom-0 w-48 h-48 bg-white/10 rounded-full blur-2xl transform translate-x-1/3 translate-y-1/3 transition-transform group-hover:scale-110"></div>
                <div className="relative z-10 flex items-center pr-10">
                  <div className="w-12 h-12 bg-white/20 rounded-2xl flex items-center justify-center mr-4 shrink-0 shadow-inner">
                    <MessageSquare className="w-6 h-6 text-white" />
                  </div>
                  <div>
                    <h3 className="text-lg font-bold mb-0.5 flex items-center">WhatsApp AI <span className="ml-2 bg-white/20 text-[10px] font-black uppercase px-2 py-0.5 rounded-full backdrop-blur-sm">FREE</span></h3>
                    <p className="text-white/90 font-medium text-xs sm:text-sm">
                      Connect your number to chat via WhatsApp instantly.
                    </p>
                  </div>
                </div>
                <div className="absolute right-6 top-1/2 -translate-y-1/2 drop-shadow-md">
              ...
              <div 
                ...
                className="cursor-pointer bg-white border border-slate-200 rounded-3xl p-6 shadow-md hover:shadow-lg transition-all group relative overflow-hidden lg:col-span-1"
              >
                ...
                <div className="relative w-16 h-16 shrink-0 flex items-center justify-center">
                  <svg className="w-16 h-16 transform -rotate-90 absolute">
                    ...
                    <circle 
                      ...
                      strokeDashoffset={28 * 2 * Math.PI * (1 - complianceScore / 100)} 
                      strokeLinecap="round" 
                      className={complianceScore === 100 ? 'text-emerald-500' : 'text-blue-500'} 
                      style={{ transition: 'stroke-dashoffset 1s ease-in-out' }}
                    />
                  </svg>
                  <div className="relative z-10 flex items-center justify-center">
                    <span className="text-sm font-black text-slate-900">{complianceScore}%</span>
                  </div>
                </div>
                <div className="min-w-0">
                  <p className="text-sm font-bold text-slate-500 truncate mb-1">Compliance Score</p>
                  <p className="text-[10px] text-slate-400 font-medium leading-tight line-clamp-2">
                    {complianceScore === 100 ? 'All docs up to date' : 'Complete profile to hit 100%'}
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col justify-center space-y-5">
                ...
                <div>
                  ...
                  <div className="flex items-center">
                    <span className="font-bold text-emerald-600 bg-emerald-50 px-3 py-1 rounded-lg text-lg tracking-widest border border-emerald-100">{taxId || 'Pending...'}</span>
                  </div>
                </div>
              </div>

              {/* Monthly Filing (VAT & WHT) */}
              {(filterCategory === 'All' || filterCategory === 'SME') && (
                <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col">
                  ...
                  <button 
                    onClick={() => navigate('/file-tax')}
                    className="text-white bg-amber-500 hover:bg-amber-600 py-3 rounded-xl font-bold flex items-center justify-center transition-colors mt-auto"
                  >
              ...
              {/* Quick File */}
              {(filterCategory === 'All' || filterCategory === 'Individual') && (
                <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm flex flex-col">
                  <div className="w-12 h-12 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center mb-5">
                    <FileText className="w-6 h-6" />
                  </div>
                  <h3 className="text-lg font-bold text-slate-900 mb-2">Personal Income Tax</h3>
                  <p className="text-slate-500 font-medium text-sm mb-6 flex-1">
                    File your PIT and claim personal reliefs securely online.
                  </p>
                  <button 
                    onClick={() => navigate('/file-tax')}
                    className="text-white bg-blue-600 hover:bg-blue-700 py-3 rounded-xl font-bold flex items-center justify-center transition-colors mt-auto"
                  >
                    Start Filing <ArrowRight className="w-4 h-4 ml-2" />
                  </button>
                </div>
        ...
        {showSessionWarning && (
           <div className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-auto">
            ...
            <motion.div 
              ...
              className="bg-white rounded-3xl w-full max-w-md mx-4 relative z-10 overflow-hidden shadow-2xl border border-slate-200"
            >
              <div className="p-6 border-b border-slate-100 flex items-center bg-red-50/50">
                <div className="w-10 h-10 bg-red-100 rounded-xl flex items-center justify-center mr-3 shrink-0">
                  <AlertCircle className="w-5 h-5 text-red-600" />
                </div>
                <h3 className="text-xl font-black text-slate-900">Session Expiring</h3>
              </div>
              <div className="p-6">
                <p className="text-sm font-medium text-slate-600 mb-6 leading-relaxed">
                  For your security, your session will automatically end due to inactivity. Are you still there?
                </p>
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col justify-center items-center mb-6">
                   <div className="text-4xl font-black text-slate-800">{sessionCountdown}s</div>
                   <div className="text-xs font-bold text-slate-400 mt-1 uppercase tracking-wider">Before Auto-Logout</div>
                </div>
                <div className="flex gap-4">
                  ...
                  <button 
                    onClick={handleStayLoggedIn}
                    className="flex-1 py-3 rounded-xl font-bold bg-blue-600 hover:bg-blue-700 text-white shadow-sm transition-colors"
                  >
                    Stay Logged In
                  </button>
                </div>
