...
import { Settings as SettingsIcon, User, Image as ImageIcon, CheckCircle, PenTool, LayoutDashboard, X, ScanFace, MessageCircle } from 'lucide-react';
      ...
      <main className="flex-1 overflow-y-auto p-6 lg:p-8">
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            {/* WhatsApp TaxBuddy AI Section */}
            <div className="bg-white rounded-3xl p-6 lg:p-8 shadow-sm border border-slate-200 space-y-6">
              <h3 className="font-bold text-slate-900 border-b border-slate-100 pb-4 text-lg flex items-center justify-between">
                <span className="flex items-center"><MessageCircle className="w-5 h-5 mr-2 text-emerald-600" /> WhatsApp TaxBuddy AI <span className="ml-3 bg-emerald-100 text-emerald-800 text-xs font-black uppercase px-2.5 py-1 rounded-full border border-emerald-300">FREE</span></span>
              </h3>
              <p className="text-sm text-slate-500 font-medium max-w-2xl">Connect your WhatsApp number to chat with TaxBuddy AI directly from your phone. Ask tax questions, generate simple invoices, and get payment links on the go.</p>
              
              <div className="flex flex-col sm:flex-row gap-4 items-end">
                <div className="w-full sm:flex-1">
                  <label className="block text-sm font-bold text-slate-700 mb-2">WhatsApp Number</label>
                  <input type="tel" placeholder="+234 800 000 0000" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-emerald-500 transition-colors" defaultValue={formData.phone} />
                </div>
                <button type="button" onClick={() => { 
                  window.open('https://wa.me/15556547445?text=Hi%20MyTaxGenius%20AI', '_blank');
                }} className="px-6 py-3 bg-[#25D366] hover:bg-[#20b858] text-white font-bold rounded-xl transition-colors whitespace-nowrap shadow-sm flex items-center">
                  <MessageCircle className="w-4 h-4 mr-2" /> Connect Bot
                </button>
              </div>

              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 mt-4 text-sm font-medium text-slate-700">
                <p><strong>Official Meta WhatsApp Cloud API (Recommended & Free):</strong></p>
                <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg text-amber-800 text-xs">
                  <strong>How to set up for free (1,000 monthly conversations):</strong>
                  <ol className="list-decimal pl-4 mt-2 space-y-1">
                    ...
                    <li><strong>Important (New Meta UI):</strong> In the left menu, click <strong>Use cases</strong> (or click "Customize the Connect with customers through WhatsApp use case" on your Dashboard).</li>
                    <li>Expand <strong>Basic setup</strong> and click <strong>Step 1. Try it out</strong>.</li>
                    <li>Copy your <strong>Access token</strong> (starts with EAA...) and save it as <strong>META_WHATSAPP_TOKEN</strong> in the Secrets panel above.</li>
                    <li><strong>CRITICAL FOR WEBHOOKS:</strong> You MUST deploy this app to Google Cloud Run (using the platform menu top right) to get a public URL. Preview URLs (`ais-pre-...`) block webhooks via a security screen.</li>
                    <li>To configure Webhooks: Close the "Use cases" menu. Scroll down the <strong>outermost left sidebar</strong> (past App settings and Alert Inbox) and look for a <strong>WhatsApp</strong> section. Click <strong>Configuration</strong>. <br/><em>(If you don't see it, go to your Dashboard, click "Add Use Case" -&gt; "Connect with customers through WhatsApp" to ensure it's fully added!)</em></li>
                    <li>Set your Webhook URL to: <br/><code className="break-all bg-amber-100 px-1 text-blue-700 font-bold">https://YOUR-CLOUD-RUN-URL/api/meta-whatsapp/webhook</code></li>
                    <li>Use the Verification Token: <strong>taxbuddy_secure_token_123</strong></li>
                    <li>Click Verify, then click Manage and subscribe to the <strong>messages</strong> webhook event.</li>
                    <li>You can now chat with the provided test phone number!</li>
                  </ol>
                </div>
                <div className="mt-3 space-y-1 text-slate-500 text-xs">
                  <p>Try sending: "What is my current PAYE balance?"</p>
                  <p>Try sending: "Create an invoice for 50k to Dan"</p>
                </div>
              </div>
            </div>
