...
export function TaxCalendarSystem() {
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    if(email) {
      ...
      setError('');
      try {
        const subId = email.replace(/[^a-zA-Z0-9]/g, '_') + '_' + Date.now();
        await setDoc(doc(db, 'reminderSubscriptions', subId), {
          email,
              ...
              <div className="p-8 bg-[#FAFAFA] dark:bg-slate-950 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-md mt-10 relative overflow-hidden">
                ...
                <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-3 relative z-10">
                  <input 
                    ...
                    className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 sm:py-0 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-emerald-500 transition-shadow shadow-sm disabled:opacity-60" 
                  />
                  <button 
                    type="submit"
                    disabled={isSubscribed || isSubmitting}
                    className="bg-slate-900 dark:bg-emerald-600 hover:bg-slate-800 dark:hover:bg-emerald-500 text-white px-6 py-3 sm:py-0 rounded-xl font-bold transition-colors whitespace-nowrap shadow-md disabled:opacity-60 disabled:cursor-not-allowed"
                  >
                    {isSubmitting ? "Subscribing..." : (isSubscribed ? "Subscribed!" : "Alert Me")}
                  </button>
                </form>
                ...
                {isSubscribed && (
                  <motion.p 
                    ...
                    className="text-emerald-600 dark:text-emerald-400 text-xs mt-4 font-bold relative z-10 flex items-center"
                  >
                    <CheckCircle className="w-4 h-4 mr-1.5" />
                    You're all set! Stay vigilant.
                  </motion.p>
                )}
              </div>
            </motion.div>
