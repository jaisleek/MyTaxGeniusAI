...
import { MessageSquare, BookOpen, Quote, Shield, Zap, Menu, X, Twitter, Linkedin, Facebook, Instagram, ChevronDown, Star, Sun, Moon, Mail, Phone, MapPin, Briefcase } from 'lucide-react';
...
export default function Layout() {
  ...
  const [language, setLanguage] = useState<'EN' | 'PG'>('EN');
  ...
  const mainNav = language === 'EN' ? [
    { name: 'Chat AI', path: '/chat', icon: MessageSquare },
    { name: 'Tax Accountant', path: '/accountant-portal', icon: Briefcase },
    { name: 'Blog', path: '/blog', icon: BookOpen },
    { name: 'New Reforms', path: '/reforms', icon: Shield },
    { name: 'Testimonials', path: '/testimonials', icon: Star },
    { name: 'Pricing', path: '/pricing' }
  ] : [
    { name: 'Yarn AI', path: '/chat', icon: MessageSquare },
    { name: 'Tax Accountant', path: '/accountant-portal', icon: Briefcase },
    { name: 'Tax Gist', path: '/blog', icon: BookOpen },
    { name: 'New Laws', path: '/reforms', icon: Shield },
    { name: 'True Talk', path: '/testimonials', icon: Star },
    { name: 'How Much', path: '/pricing' }
  ];

  const toolsNav = language === 'EN' ? [
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] dark:bg-slate-950 flex flex-col font-sans relative selection:bg-emerald-200 selection:text-emerald-900 transition-colors duration-300">
      <div className="sticky top-0 z-50 pointer-events-none w-full h-0" style={{ marginBottom: 0 }}>
        <div className="pointer-events-auto h-auto">
          <DeadlineCountdown />
        </div>
        {/* Floating Island Header */}
        <div className="px-4 sm:px-6 pointer-events-none mt-2 transition-all duration-300">
          <header className="max-w-7xl mx-auto bg-[#0F172A]/95 dark:bg-slate-950/90 backdrop-blur-3xl rounded-2xl lg:rounded-full shadow-[0_8px_30px_rgba(0,0,0,0.12)] border border-white/10 dark:border-slate-800/50 transition-all duration-300 pointer-events-auto">
          <div className="px-4 md:px-6 flex justify-between h-16 lg:h-20 items-center">
            ...
            <Link to="/" className="flex items-center space-x-3 group cursor-pointer z-50 lg:mr-8">
              ...
              <span className="font-extrabold text-xl sm:text-2xl tracking-tight text-white transition-colors">
                MyTaxGenius
              </span>
            </Link>

            {/* Desktop Navigation */}
            <nav className="hidden xl:flex items-center space-x-0.5 xl:space-x-1 flex-1">
              ...
              <div className="relative group">
                <button 
                  ...
                  className={cn(
                    "px-2 xl:px-3 py-2.5 rounded-full text-sm font-bold transition-colors flex items-center space-x-1 whitespace-nowrap",
                    isToolsDropdownOpen 
                      ? "text-emerald-400 bg-emerald-900/40" 
                      : "text-slate-200 hover:bg-slate-800 hover:text-white"
                  )}
                >
                  <span>{language === 'EN' ? 'Features' : 'Wetin Dey'}</span>
              ...
              {mainNav.map((item) => {
                ...
                return (
                  <Link
                    ...
                    to={item.path}
                    className={cn(
                      "px-1.5 xl:px-3 py-2.5 rounded-full text-sm font-bold transition-all duration-300 items-center space-x-1 whitespace-nowrap flex",
                      isActive
                        ? "text-emerald-400"
                        : "text-slate-300 hover:text-white hover:bg-slate-800"
                    )}
                  >
                    {isActive && <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 shrink-0"></div>}
                    <span>{item.name}</span>
                  </Link>
                );
              })}
            ...
            <div className="hidden xl:flex items-center space-x-2 xl:space-x-3 shrink-0">
              <button 
                onClick={toggleTheme}
                className="w-10 h-10 flex items-center justify-center rounded-full bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-white transition-colors"
              ...
              <Link
                to="/login"
                className="text-sm font-extrabold text-slate-200 hover:text-white transition-colors px-2 whitespace-nowrap"
              >
                Sign In
              </Link>
              <Link
                to="/signup"
                className="px-5 py-2.5 rounded-full text-sm font-extrabold transition-all duration-300 bg-white text-slate-900 hover:bg-slate-200 shadow-md whitespace-nowrap"
              >
                Create Account
              </Link>
            </div>
        ...
        <div className={cn(
          "xl:hidden absolute top-[calc(100%+10px)] left-0 right-0 bg-white dark:bg-slate-950 border border-slate-100 dark:border-slate-800 shadow-2xl rounded-2xl transition-all duration-300 ease-in-out origin-top z-50 p-4 pointer-events-auto",
          isMobileMenuOpen ? "opacity-100 scale-y-100" : "opacity-0 scale-y-0 pointer-events-none"
        )}>
          <div className="flex flex-col space-y-2 pb-6 max-h-[calc(100vh-8rem)] overflow-y-auto">
            <div className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-4 mb-2 mt-4">{language === 'EN' ? 'Features' : 'Wetin Dey'}</div>
            {toolsNav.map((item) => (
              <Link
                key={item.path}
                to={item.path}
            ...
            <div className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-4 mb-2 mt-6">{language === 'EN' ? 'Resources' : 'Help'}</div>
            {mainNav.map((item) => (
              <Link
                key={item.path}
                to={item.path}
